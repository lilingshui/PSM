...
class CDlgReadAlarmData : public CDialog, public CEventRecver, public CChannelReceiver
{
	DECLARE_DYNAMIC(CDlgReadAlarmData)
...
public:
	CDlgReadAlarmData(CWnd* pParent = NULL);   // 标准构造函数
	virtual ~CDlgReadAlarmData();

// 对话框数据
	enum { IDD = IDD_FORM_READ_ALARM_DATA };
...
public:
	...
	CCheckListCtrl m_ctrlTermAlarmItem;

	CDateTimeCtrl m_ctrlDateStart;
	CDateTimeCtrl m_ctrlTimeStart;
	CDateTimeCtrl m_ctrlDateEnd;
	CDateTimeCtrl m_ctrlTimeEnd;
	afx_msg void OnNMDblclkListOptAlarmResult(NMHDR *pNMHDR, LRESULT *pResult);
	afx_msg void OnBnClickedBtnAlarmStop();
};
