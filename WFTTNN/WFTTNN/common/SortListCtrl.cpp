...
BOOL CSortListCtrl::SetColumns( const CString& strColumns )
{
  int iStart = 0;
  
  for( ;; )
  {
    const int iComma = strColumns.Find( _T(','), iStart );
    
    if( iComma == -1 )
      break;
    
    const CString strHeading = strColumns.Mid( iStart, iComma - iStart );
    
    iStart = iComma + 1;
    
    int iSemiColon = strColumns.Find( _T(';'), iStart );
    
    if( iSemiColon == -1 )
      iSemiColon = strColumns.GetLength();
    
    const int iWidth = atoi( strColumns.Mid( iStart, iSemiColon - iStart ) );
